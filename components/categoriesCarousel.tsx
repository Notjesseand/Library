...
const categoriesCarousel = () => {
  return (
    <Swiper
      ...
      className="mySwiper  font-open px-24"
    >
      {categories.map((item, index) => (
        <SwiperSlide
          key={index}
          className=" px-24 mt-12 h-[40px] inline-block cursor-pointer  text-white text-center w-full backdrop-blur-sm rounded-sm    "
          style={{ backgroundImage: "/drama.jpg" }}
        >
          <div className="relative group h-44 sm:h-56 bg-gray-500 bg-cover w-36 sm:w-5/6 bg-center mx-auto rounded overflow-hidden transition-opacity duration-300">
